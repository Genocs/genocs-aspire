...
internal class AzureAdClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
    ...
    public override async ValueTask<ClaimsPrincipal> CreateUserAsync(RemoteUserAccount account, RemoteAuthenticationUserOptions options)
    {
        ...
        if (principal.Identity?.IsAuthenticated is true)
        {
            ...
            if (!string.IsNullOrWhiteSpace(userDetails.ImageUrl) && !userIdentity.HasClaim(c => c.Type == GNXClaims.ImageUrl) && userDetails.ImageUrl is not null)
            {
                userIdentity.AddClaim(new Claim(GNXClaims.ImageUrl, userDetails.ImageUrl));
            }

            var permissions = await _services.GetRequiredService<IPersonalClient>().GetPermissionsAsync();

            userIdentity.AddClaims(permissions.Select(permission => new Claim(GNXClaims.Permission, permission)));
        }
